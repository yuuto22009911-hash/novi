      ...
      <tbody>
        {found.variables.map((variable) => (
          <tr key={variable.name} className="border-b border-site-border align-top">
            <td className="py-2 pr-4 font-mono text-xs whitespace-nowrap">{variable.name}</td>
            <td className="py-2 pr-4 font-mono text-xs text-site-muted">{variable.value}</td>
            {hasDark && (
              <td className="py-2 font-mono text-xs text-site-muted">{variable.dark ?? '同じ'}</td>
            )}
          </tr>
        ))}
      </tbody>
    ...
    <Preview className="gap-3">
      <ul className="flex flex-wrap gap-3">
        {colors.variables.map((variable) => (
          <li key={variable.name} className="flex w-32 flex-col gap-1">
            <span
    ...
    <ul className="flex flex-col gap-3 text-sm leading-relaxed">
      {designRules.prohibited.map((rule) => (
        <li key={rule.id} className="flex flex-col gap-0.5">
          ...
          <span className="text-site-muted">{rule.reason}</span>
        </li>
      ))}
    </ul>
  )
