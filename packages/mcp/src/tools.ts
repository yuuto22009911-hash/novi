 ...
 */
import { type ComponentEntry, index } from './index-data.js'

const { components, themes, tokenTypes, version } = index
...
export function listComponents(): string {
  const rows = components.map((component) => {
    const where = isAvailable(component)
      ? component.implementedBy.map((key) => themes[key]?.pkg ?? key).join(', ')
      : '**未実装**'
...
/** 1コンポーネントの詳細（AC-04-2）。 */
export function getComponent(name: string, theme?: string): string {
  ...
  const themeKey = theme ?? component.implementedBy[0]
  const pkg = themeKey === undefined ? '@novi-ui/raster' : (themes[themeKey]?.pkg ?? themeKey)
...
export function getDesignRules(theme: string): string {
  ...
  const prohibited = designRules.prohibited.map((rule) => `- \`${rule.pattern}\` — ${rule.reason}`)
...
export function searchComponents(query: string): string {
  ...
  const matched = components
    .filter(isAvailable)
    .map((component) => {
      const terms = [component.name, ...component.keywords].filter((term) => hits(term, normalized))
