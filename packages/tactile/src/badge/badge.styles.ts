...
const size: VariantMap<NoviSize, { root: string; dot: string }> = {
  ...
  // 文字だけは読める大きさを保つ
  sm: { root: 'h-6 px-[var(--novi-pad-control-x-sm)] text-[length:var(--novi-text-xs)]', dot: 'size-1.5' },
  md: { root: 'h-7 px-[var(--novi-pad-control-x-md)] text-[length:var(--novi-text-sm)]', dot: 'size-2' },
  lg: { root: 'h-8 px-[var(--novi-pad-control-x-lg)] text-[length:var(--novi-text-sm)]', dot: 'size-2.5' },
}
