...
describe('Button: variant / size / color（AC-02-1 / AC-01-2）', () => {
  ...
  it.each([
    ...
    ['md', 'h-10'],
    ['lg', 'h-12'],
  ] as const)('size=%s の高さが %s（32/40/48px）', (size, expected) => {
...
describe('Button: 拡張（AC-06-1 / AC-06-2）', () => {
  it('tv({ extend }) で拡張でき、元の variant が維持される', () => {
    ...
    expect(extended).toContain('h-12')
...
describe('Button: Raster のデザイン規律', () => {
  it('影を使っていない', () => {
    const all = NOVI_VARIANTS.map((variant) => buttonStyles({ variant }).root()).join(' ')
    expect(all).not.toMatch(/\bshadow-(?!none)/)
  })

  it('scale / rotate を使っていない', () => {
    const all = NOVI_VARIANTS.map((variant) => buttonStyles({ variant }).root()).join(' ')
    expect(all).not.toMatch(/\b(scale|rotate)-/)
  })

  it('色をトークン経由で参照している（リテラル色を書いていない）', () => {
  ...
  it('モーションがトークン経由になっている', () => {
    expect(buttonStyles().root()).toContain('var(--novi-duration-fast)')
  })
})
